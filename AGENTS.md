...
- User-approved local validation policy (2026-09-11): checks relevant to the change may run locally and, together with the required Terra review, satisfy the merge gate without a successful hosted Actions run. Follow `docs/decisions/local-validation-policy.md`; record the reviewed revision, commands, tool versions, results and applicability rationale in the ticket or PR. Runner provisioning failures are unavailable evidence, never passed tests. Actual relevant test failures still block merge. Platform changes still require their platform checks; hardware and independent security gates remain mandatory. Do not bypass remote branch protection or change account settings as a side effect.
- User-approved physical acceptance scheduling (2026-09-14): implement and run automated/native checks first; defer real-device certification until integrated candidates under the policy's explicit MC-025/027/043/044 ownership and ticket dependencies. Implementation completion is not hardware certification. Physical gates remain required before their beta/release descendants; native compilation checks and independent assessments are not deferred. Never mark a transferred physical scenario passed without device evidence.
