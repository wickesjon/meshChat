...
Local Rust 1.85.1 all-feature Clippy and Debug/Release tests pass: four existing foundation tests plus three curve/vector/input tests. Cargo-deny 0.20.2 advisories, bans, licenses and sources pass (only existing unused-license warnings). Host Kotlin/Swift binding generation passes. Android Debug/Release Kotlin compilation and lint pass against upstream 4.17.0; the final rebuilt AAR awaits hosted CI. At `85047be`, CI run `34634014837` passed all four unsigned iOS security builds (Debug/Release, simulator/device) and Swift/CryptoKit host interoperability; hosted Rust and ticketboard jobs also passed. Workflow syntax validation passes. These are automated results, not security/device acceptance.

The user requested emulator testing on 2026-09-11. A custom Android API 29 instrumentation runner executes three process-separated phases for creation, persistence/wrong-key reopen, and key-loss/refusal/reset. It checks actual database data and retained ciphertext without logging contents. At `0741b26`, run `34634940919` passed the rebuilt SQLCipher APK alignment and native builds/lint, including instrumentation compilation. Device-manager setup failed before emulator execution because it inferred its host SDK location; the workflow now copies its tools into the repository-local SDK. After the SDK fix, run `34636287902` at `12c30af` passed all build/alignment checks but timed out waiting for emulator ADB connection; no lifecycle phase ran. Startup now precedes expensive builds and checks process liveness with logged diagnostics and a bounded boot deadline. Hosted emulator execution remains pending; no physical requirement is replaced.
