...
MC-022 assessment clarification: public signed CHAT replay identity excludes the local incoming/outgoing history direction. The same broadcast returning to its author is a replay, including after reopening or deleting visible history. The storage owner writes public tombstones under canonical direction zero and checks both existing direction-zero and direction-one rows without deleting them; conflicting legacy digests refuse new effects. DM replay identity still includes direction. History retains its presentation direction. This corrects acceptance behavior without changing the database schema or wire format; running an older vulnerable binary is not a replay-protection guarantee.

The mandatory forbidden-character check belongs to Rust acceptance owners. Friend and organizer owners validate original CHAT nickname/text and signed ANNOUNCE nickname after signature authentication, before new verified history/attribution effects; invalid text resolves only that exact pending variant as rejected. Unsigned clear CHAT/ANNOUNCE/EVENT_INFO is checked by ingress before accepted dedup/effects. Encrypted and unknown content remains opaque there; DM plaintext retains its post-authentication check. These checks never normalize signed bytes. Native presentation must still use sanitized display copies, badge-safe chrome, combining-mark limits and confusable warnings. A pin outside its signed authority lifetime is suppressed while otherwise valid authenticated text retains its status, as already required by MC-006.
