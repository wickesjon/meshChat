...
Status: exact dependency exceptions approved by the user on 2026-09-11. The [MC-008 crypto contract](MC-008-crypto-contract.md) selects the construction. This record authorizes only the listed version pairs at their later integration; it does not add production dependencies or claim a DM implementation or independent security-review pass.
...
Select RFC 9180 Auth mode with a fixed X25519/HKDF-SHA256/ChaCha20Poly1305 suite and `hpke = "=0.14.1"` as the baseline for MC-008 under the user-approved exact duplicate-version exceptions below. This preserves the existing platform probe's pins and selects the current release rather than an older release solely to reduce duplicates. The crypto contract specifies application inputs; platform implementation and independent assessment still need their owning work.
...
The user authorized only coexistence of these resolved version pairs for the selected HPKE 0.14.1 graph:
...
Apply exact-version cargo-deny exceptions only when adding that dependency in the owning implementation ticket (MC-020, with MC-017 provider integration as needed). Keep `multiple-versions = "deny"` for everything else, no wildcard/subtree skips, and preserve advisory/license/source gates. An exact skip for the existing version alone does not constrain its companion version: additionally verify the active all-target/all-feature graph contains only the approved pairs, rejecting any new pair/version. Review dependency upgrades and removal/consolidation of these exceptions explicitly; this approval does not permit arbitrary future versions or a broad dependency refresh. Re-resolve against the then-current main lockfile and obtain a new scope decision if different pairs are needed.
...
After approval, the isolated experiment compared the distinct versions in `cargo tree --all-features --locked --target all --prefix none --format '{p}'` against exactly the nine pairs. It passed. Using a repo-local copy of deny.toml with exact skips for the existing versions then passed advisories, bans, licenses and sources. The commands use the same experiment manifest; the retained script/config/log are `.work/mc008-dependency-014/check_approved_pairs.py`, `approved-deny.toml` and `approved-deny.log`. Raw lockfile or metadata package inventories also contain inactive optional dependencies; they are not the active feature graph. This successful isolated policy experiment does not claim that the future production integration has already passed.
