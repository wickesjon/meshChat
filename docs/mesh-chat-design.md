...
MC-019/020/021 now provide implemented, tested friend/DM/organizer verification. The [MC-022 review packet](testing/MC-022-crypto-review-packet.md) maps the combined wire/QR definitions, executable evidence and assessment questions. Full-wire freeze remains pending the independent construction/transcript assessment and resolution of its findings; reference agreement and Kotlin/Swift parity do not replace it.
...
6. **Messages tab**: 1:1 DM threads (petname-titled), each with the closed-lock indicator and "Encrypted" preview styling; FAB → "New message" (picks from pinned friends — encryption requires a held key). A user-confirmed pin replacement closes the old DM sending context and starts the replacement flow in §7.4; a network-observed new key alone never blocks the existing pinned conversation. The list design reserves room for group threads (group-name-titled, member-count cue) so v2 encrypted group chats (§20) slot in without a layout change.
