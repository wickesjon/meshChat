...
8. **Evidence and sequencing.** MC-004 uses the explicitly approved online BLE feasibility gate below; the two-platform key-storage spike remains a foundations gate. Android beta depends explicitly on the full crypto gate. Phone Beacon Mode is v1; ESP32/backhaul are follow-on work. A design-stage fix is only specified until implemented, tested, and independently reviewed where required.
