...
1. **Create event:** enter the event name (at most 32 UTF-8 bytes) and an explicit UTC expiry in `YYYY-MM-DD HH:MM` form. Check inputs without generating a key, then confirm the actual name and expiry. Choose a new encrypted `.mcvault` file; an existing file is never overwritten.
...
The vault is a hex encoding of `meshchat/offline-root/v1\0`, a fresh 12-byte nonce, and ChaCha20-Poly1305 ciphertext/tag. The exact magic is authenticated as associated data. Plaintext is a 32-byte root seed, four-byte big-endian root expiry and the validated UTF-8 event name. Every creation uses independent operating-system randomness for the root seed, 32-byte unlock key and nonce. Opening checks authentication and expiry before signing. The root private seed never crosses the Rust-to-console boundary. Mutable metadata and staff issuance use the existing MC-006/008 wire grammar and domains unchanged.
