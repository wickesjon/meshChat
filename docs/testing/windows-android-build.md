...
MC-046 adds Windows host support to the Android path in `src/core/build_bindings.py`. This guide covers Rust libraries/bindings and the skeleton app's build, lint and JVM FFI checks. It does not claim full security-probe CI, emulator execution, physical-device evidence or iOS validation.
...
& ./src/android/gradlew.bat -p src/android --no-daemon :app:assembleDebug :app:assembleRelease :app:lintDebug :app:testDebugUnitTest
...
python -B tests/integration/ffi/check_android_apk.py src/android/app/build/outputs/apk/debug/app-debug.apk src/android/app/build/outputs/apk/release/app-release-unsigned.apk
...
The JVM test exercises the Windows host library through generated Kotlin/JNA, while packaging checks inspect the ARM64/x86-64 ELF libraries shipped in both APKs. Neither runs those libraries on an Android emulator/device. The release APK here is unsigned, not a distribution artifact.
