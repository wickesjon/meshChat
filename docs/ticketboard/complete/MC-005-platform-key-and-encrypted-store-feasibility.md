...
Run `34649412625` at `2f86859` passed Rust, ticketboard, iOS and Android build/lint/alignment checks but still refused userdata creation (7372.80 MB required despite the 2 GiB setting). No lifecycle phase ran. The follow-up retains the emulator's 6 GiB requirement and frees only four repository-local CI build caches after APK validation, with Linux-CI/workspace/path guards and an 8 GiB free-space preflight. Five mocked cleanup boundary tests pass locally. See the probe plan for the source analysis; fresh CI and Terra review remain required before merge. No host tools, test APKs, lifecycle assertions or protection settings are removed or weakened.

Run `34650732080` (`12dc1f0`) passed native checks and five cleanup boundary tests, then correctly refused emulator boot because reclaimed space (7,696,547,840 bytes) remained below the 8 GiB preflight. The next revision also frees this job's repository-local `.work/rustup` and `.work/cargo` after compilation; packaged APKs and host tools remain intact. The six-path allowlist retains the same guards and threshold. No lifecycle result is claimed from the failed preflight; fresh CI and Terra review remain required.
