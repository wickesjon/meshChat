...
- [x] Relevant checks pass, evidence is recorded and required review is complete; ticket completion becomes effective only when the final reviewed squash commit lands on main.
...
Dependency changes are covered by the native-consumer row of the approved local-validation policy. At scope approval, the Windows environment had no Mac/Xcode and hosted Actions provisioning was unavailable under the previously recorded account limitation. Scope approval did not waive native checks. The later successful hosted rerun below supplies that evidence; no host probe is described as native evidence and no hardware/independent-security gate is waived.

The user approved adding root `Cargo.lock` to MC-011 scope on 2026-09-11. The exact dependency integration above is authorized; native validation remained required and subsequently passed as recorded below. Completion becomes effective on main after the reviewed squash merge.
...
### Native dependency validation history
...
That local result is Android cross-compilation/app packaging and host JVM FFI evidence, not emulator, physical-device or full SQLCipher/security-app lifecycle validation. At that stage, Unix tooling and Mac/Xcode validation were unavailable, so MC-011 stayed inreview. The successful hosted rerun below subsequently closed those automated native gaps. No production native UI is wired by this internal module. MC-045's physical deferral did not waive native compilation; independent security gates remain unchanged.

## Review and merge

### Hosted validation resumed after public visibility

On 2026-09-15 the user made the repository public; the repository API confirms public visibility. The failed jobs in [run 34937556942](https://github.com/wickesjon/meshChat/actions/runs/34937556942) were rerun on unchanged revision `1ae25d44d29567a526215418b3f167c3b9e3c7cf` and successfully provisioned Linux and Mac runners. The earlier runner-access limitation is resolved for this attempt.

Rust job `104285684482` and ticketboard job `104285684792` passed. iOS job `104285684797` passed with Xcode 16.4 build 16F6: Rust iOS libraries/bindings, Swift host FFI lifecycle/binary/error regression, unsigned app and BLE/security probe builds, and the Swift/CryptoKit Ed25519/X25519 interoperability regression. This supplies actual iOS build/interop evidence; the earlier unavailable-iOS statements above describe the prior local-only state. It does not supply physical-device/key-storage evidence.

Android job `104285684712` passed its native Rust, app/lint/packaging, BLE and SQLCipher/security build stages and the emulator lifecycle stage. All four jobs in the rerun are successful on the unchanged MC-011 source revision. The previously unavailable native evidence is now available; physical-device acceptance and independent security assessments remain their later separate gates. The final ticket/index-only completion update receives follow-up Terra review before squash merge, under the local-validation policy for documentation-only amendments.

Android logs confirm pinned SQLCipher revision `ae57a61052d8c41ce35cd48319b2f6f20f4de6bf` rebuilt for both ABIs; debug/release SQLCipher/Rust/JNA APKs pass 16 KB LOAD/RELRO checks. The API 29 emulator reports 4096-byte pages and explicit PASS results for synthetic `create`, `reopen` and `key-loss` phases, ending with persistence/wrong-key/key-loss/reset success. This 4 KB emulator result does not claim 16 KB runtime execution, physical backup/lock acceptance, or real-device protection certification.

- Branch: `ticket/MC-011-channels-text-and-deep-link-parsing`.
- PR: [#13](https://github.com/wickesjon/meshChat/pull/13).
...
- The initial review above covers MC-011 source; main integration brought separately reviewed MC-045/046 changes, and [review 5206399962](https://github.com/wickesjon/meshChat/pull/13#pullrequestreview-5206399962) covers integrated revision `1ae25d44d29567a526215418b3f167c3b9e3c7cf`. All required hosted jobs now pass on that revision. Final completion-metadata review is recorded on the PR before squash; no code, dependencies or validation behavior changed after that passing run.
