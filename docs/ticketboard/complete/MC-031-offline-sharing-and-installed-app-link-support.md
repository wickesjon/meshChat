...
Implemented from main `81170ad54b481f483f3227f138147e2a4d2ef2d6`; MC-011/028 and reused MC-029 are complete there. See [implementation/validation evidence](../../sharing/MC-031-validation.md) and [sharing/activation guide](../../sharing/README.md). All relevant local core, template, Android build/lint/JVM, APK alignment and emulator checks pass, including cache refusal/expiry, protected friend/DM regressions and small-viewport channel/sharing flows. The hosted iOS job passed at `6a3d51e` with identical shared/Swift inputs to the final revision. The second criterion uses the ticket's explicit missing-identifiers fallback; it does not claim production association.

The previously pending MC-029 hosted Android run failed because its UI fixture addressed an offscreen, uncomposed LazyColumn item. Its saved screenshot and stack were inspected. The click helper now scrolls the container first, and all baseline/sharing phases pass on the matching 320×640 viewport. This is regression-harness remediation in the approved test path; no product invariant or assertion was weakened.
...
- Branch: `ticket/MC-031-offline-sharing-and-installed-app-link-support`.
- Review/PR: [PR #32](https://github.com/wickesjon/meshChat/pull/32). Separate Terra medium source review approved `6a3d51e7e0f5c7f287cd92c3751a13243476f7e6` with no blocking findings. Final fixture/evidence review and exact reviewed revision are recorded in the PR body before merge; CI is not substituted for review.
- Squash commit title: `MC-031: Offline sharing and installed-app link support`.
- Completion becomes effective only when the reviewed squash commit lands on main.

The final combined exit checkbox remains unchecked in this pre-merge snapshot because it includes the squash merge itself. The `complete/` path is staged for that squash; until review and merge finish it is pending, not effective completion. After merge, the main-branch ticket location and merged PR establish completion.
