...
The user requested emulator testing. Three process-separated Android API 29 phases implement create/reopen/wrong-key, key-loss refusal/ciphertext retention and explicit reset. The SDK-location issue was fixed, but run `34636287902` timed out waiting for emulator ADB connection: **no lifecycle runtime pass is claimed**. Startup now runs before expensive builds, records acceleration/AVD diagnostics, detects emulator process exit immediately and retains failure output. Current emulator verification remains pending. No physical gate was waived.
