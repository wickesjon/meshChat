...
### Remaining merge blocker: native dependency checks

The Android/iOS native-consumer checks are still required by the approved policy because this change adds shared dependencies. Windows is not a supported host in `build_bindings.py`'s Android NDK compiler mapping, and this environment has no `xcodebuild`/`xcrun`. These checks are unavailable, not passed. No production native UI is wired by this internal module; later native confirmation/rendering work is also not claimed here. The PR may receive the required separate Terra review, but this ticket stays inreview and cannot squash merge until native validation is available or the user explicitly changes that policy. Physical and independent security gates remain unchanged.
