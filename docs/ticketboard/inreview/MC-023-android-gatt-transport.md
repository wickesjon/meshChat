...
After this correction, local `assembleDebug assembleRelease lintDebug` passes with all 14 JVM cases and both APK ABI/alignment checks. The change is limited to Android callback handling, its regression and evidence; Rust and Swift sources are unchanged from the initial candidate. Follow-up Terra review remains required before merge.
