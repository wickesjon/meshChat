...
for phase in ['purchase', 'reopen-refund']:
    ...
    result = run('shell', 'am', 'instrument', '-w', '-r', '-e', 'phase', phase,
                 '-e', 'class', 'org.meshchat.ui.BillingUiTest',
                 'org.meshchat.app.test/androidx.test.runner.AndroidJUnitRunner').decode()
    print(result, flush=True)
    for name in ['violet-flair', 'theme-afterhours', 'theme-daylight', 'theme-ember', 'theme-lagoon', 'theme-violet']:
        data = run('exec-out', 'run-as', 'org.meshchat.app', 'cat', 'files/mc032-'+name+'.png')
