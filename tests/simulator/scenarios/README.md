...
[Versioned SYNC selection fixtures](MC-007-sync-fixtures.json) pin every input needed for the mixed-channel and deterministic-Bloom cases: subscriptions, held ID values/range, all 512 Bloom bytes, newest-first cache records, selected/omitted IDs, pages/marker flags and display outcomes. The definition validator recomputes the filter using the normative SHA-256 double-hash and MSB-first bit order, then checks each literal expected result.

The mixed-channel fixture selects IDs 101–108, including two records for an unsubscribed private channel; selection cannot assume subscriptions. The fixed-filter fixture holds IDs 1–500 and includes missing ID 525 (`000000000000020d`), which is a deterministic false positive. Missing IDs 501 and 502 are selected; ID 525 remains omitted in two separately admitted sessions with the same injected filter. The fixture explicitly controls the filter input rather than asserting normal cache recomputation is unchanged. Repeated selected items are exact duplicates at the receiver, so only the first walk adds display entries. Report two of three missing IDs recovered, alongside the two-item selected-set denominator and valid terminal marker. A completed filtered walk does not mean every missing message was recovered.
