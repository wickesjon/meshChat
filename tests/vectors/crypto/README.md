...
- [CFRG HPKE test-vector repository](https://github.com/cfrg/draft-irtf-cfrg-hpke): retain the exact commit and source-file SHA-256 when importing permitted public fixtures. The selected crate also ships `test-vectors-5f503c5.json`; inspect provenance and suite/mode before use. A moving branch is a discovery link, not a pinned evidence record.
