...
## MC-021 organizer reference fixtures

`organizer_vectors.cjs` uses Node/OpenSSL pure Ed25519 independently of Rust/dalek. It first reproduces RFC 8032 §7.1 TEST 1 (the same published public test material in `tests/bench/security/curves.rs`), then writes `organizer-v1.tsv`: canonical root bundle, two staff credentials, included/omitted/pinned posts, and a valid signature over the wrong domain. Regenerate with `node tests/vectors/crypto/organizer_vectors.cjs > .work/organizer-vectors.tsv`; compare its bytes with the committed fixture. Generation used Node 24.15.0/OpenSSL 3.5.5. Only deterministic/public synthetic test keys are used. No actual provisioning URL, private event key or QR image is retained.

`tests/integration/organizer/organizer.rs` compares byte-exact output and exercises production strict verification, protected staff signing, all immutable-byte mutations, root/credential/time/pin bindings, included/ambiguous credential handling, adopted-root transitions, durable replay/conflict, failures/eviction/concurrency/budget recovery and multi-hop credential flooding. Root-hint collisions use a synthetic already-filtered candidate bucket with the production full-key resolver; no real truncated-hash collision is claimed. The simulator executes the same admitted production harness. Reference agreement and automated peer review do not replace MC-022's independent construction/transcript assessment or native feature/physical certification.
